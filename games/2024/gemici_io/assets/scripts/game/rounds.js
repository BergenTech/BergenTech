        ...
        if (round50Time >= 60 * 60) {
            ...
            if (round50Time === 60 * 60) {
                for (let i = 0; i < enemies.length; i++) {
                    ...
                    enemy.dropChance = 0;
                    enemy.kill();
                    enemies.splice(i, 0, null);
                }

                game.enemies.enemies.splice(0, Infinity);
                
                game.objects.spawnObject("spawner", [1500, 1500]);
            }
...
export default function runRounds(game) {
    if (game.screen === "pause" || game.screen === "over" || game.screen === "win") {
        return false;
    } else if (game.player.dead === true) {
